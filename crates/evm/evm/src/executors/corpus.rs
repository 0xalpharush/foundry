...
enum MutationType {
    ...
    Delete,
    /// Mutate warp (time delay) of random calls in the sequence.
    Warp,
    /// Mutate roll (block delay) of random calls in the sequence.
    Roll,
}
...
pub struct WorkerCorpus {
    ...
    /// Maximum time delay (seconds) for warp mutation. None = no warp mutation.
...
pub struct SyncStats {
    /// Number of entries imported (from other workers' sync dirs).
    pub imported: usize,
    /// Number of entries exported (to other workers' sync dirs).
    pub exported: usize,
    /// Time spent calibrating (importing/replaying sync entries).
    pub calibrate_elapsed: std::time::Duration,
    /// Time spent exporting entries to other workers.
...
impl WorkerCorpus {
    pub fn new(
        ...
        let mut mutation_types: Vec<BoxedStrategy<MutationType>> = vec![
            ...
            Just(MutationType::Repeat).boxed(),
            Just(MutationType::Interleave).boxed(),
            Just(MutationType::GenPrefix).boxed(),
            Just(MutationType::GenSuffix).boxed(),
            Just(MutationType::MutatePrefix).boxed(),
            Just(MutationType::MutateSuffix).boxed(),
            Just(MutationType::GenMutate).boxed(),
            Just(MutationType::Swap).boxed(),
            Just(MutationType::Delete).boxed(),
        ];
        if max_time_delay.is_some() {
            mutation_types.push(Just(MutationType::Warp).boxed());
        }
        if max_block_delay.is_some() {
            mutation_types.push(Just(MutationType::Roll).boxed());
        }
        let mutation_generator = proptest::strategy::Union::new(mutation_types).boxed();
        ...
        let in_memory_corpus: Vec<CorpusEntry> = vec![];
        let history_map = vec![0u8; COVERAGE_MAP_SIZE];
        let top_rated = vec![None; COVERAGE_MAP_SIZE];
        let metrics = CorpusMetrics::default();
        let failed_replays = 0;
    ...
    pub fn new_inputs(
        ...
        if !self.in_memory_corpus.is_empty() {
            ...
            match mutation_type {
                ...
                MutationType::Warp => {
                    trace!(target: "corpus", "warp mutate {}", primary.uuid);

                    self.current_mutated = Some(primary.uuid);
                    new_seq = primary.tx_seq.clone();
    ...
    fn export_to_master(&self) -> Result<usize> {
    ...
    #[instrument(skip_all)]
    /// Returns the number of unique entries exported (distributed to at least one worker).
    fn export_to_workers(&mut self, num_workers: usize) -> Result<usize> {
        ...
        debug!(target: "corpus", exported, "distributed master corpus to all workers");
    ...
    pub fn sync(
        ...
        let calibrate_elapsed = cal_timer.elapsed();

        let exp_timer = std::time::Instant::now();
        let exported = if self.id == 0 {
            self.export_to_workers(num_workers)?
        } else {
            self.export_to_master()?
        };
        let export_elapsed = exp_timer.elapsed();
...
mod tests {
    ...
    fn new_empty_manager() -> WorkerCorpus {
    ...
    fn top_rated_tracks_smallest_corpus_per_edge() {
        let mut manager = new_empty_manager();
    ...
    fn cull_queue_selects_minimal_covering_set() {
        ...
        manager.in_memory_corpus = vec![corpus_a, corpus_b, corpus_c];
    ...
    fn eviction_updates_top_rated() {
        let mut manager = new_empty_manager();
        manager.config = Arc::new(FuzzCorpusConfig {
            corpus_dir: Some(temp_corpus_dir()),
            corpus_gzip: false,
            corpus_min_mutations: 0,
            corpus_min_size: 1,
        ...
        manager.in_memory_corpus = vec![corpus_a, corpus_b];
    ...
    fn eviction_skips_favored_and_evicts_non_favored() {
        ...
        let favored = CorpusEntry::with_edges(vec![basic_tx()], vec![0]);
        let favored_uuid = favored.uuid;

        let non_favored = CorpusEntry::with_edges(vec![basic_tx(); 2], vec![0]);

        manager.in_memory_corpus = vec![favored, non_favored];
